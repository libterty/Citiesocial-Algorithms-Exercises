      ...
      if ((last - next) % (next - start) !== 0) {
        ...
        while (i < 10) {
          ...
          result.length === 0
            ...
            : result.push(result[result.length - 1] + (result[result.length - 1] - result[result.length - 2]) + diff);
          i++;
        }
      }
  
      // 多階等差數列
      if ((last - next) / (next - start) > 1 && (last - next) % (next - start) === 0) {
        while(i < 10) {
          result.length === 0
            ? result.push(last + ((last - next) / (next - start) * (last - next)))
            : result.length === 1
            ? result.push(result[0] + ((result[0] - last) / (last - next)) * (result[0] - last))
            : result.length === 2
            ? result.push(result[result.length - 1] + ((result[result.length - 1] - result[result.length - 2]) / (result[result.length - 2] - last) * (result[result.length - 1] - result[result.length - 2])))
