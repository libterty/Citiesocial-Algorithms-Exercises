...
export default function fib(n) {
    ...
    if (n.length <= 2) {
      ...
      if (n[n.length - 1] - n[n.length - 2] === n[n.length - 2] - n[n.length - 3]) {
        ...
        while(i < 10) {
          result.length === 0 
            ? result.push(n[n.length - 1] + (n[n.length - 2] - n[n.length - 3]))
            : result.push(result[result.length - 1] + ((n[n.length - 2] - n[n.length - 3])));
          i++;
        }
      }
  
      // 多階等差數列
      if ((n[n.length - 1] - n[n.length - 2]) / (n[n.length - 2] - n[n.length - 3]) > 1) {
        console.log('多階等差數列')
        while(i < 10) {
          result.length === 0
            ? result.push(n[n.length - 1] + ((n[n.length - 1] - n[n.length - 2]) / (n[n.length - 2] - n[n.length - 3]) * (n[n.length - 1] - n[n.length - 2])))
            : result.length === 1
            ? result.push(result[0] + ((result[0] - n[n.length - 1]) / (n[n.length - 1] - n[n.length - 2])) * (result[0] - n[n.length - 1]))
            : result.length === 2
            ? result.push(result[result.length - 1] + ((result[result.length - 1] - result[result.length - 2]) / (result[result.length - 2] - n[n.length - 1]) * (result[result.length - 1] - result[result.length - 2])))
            : result.push(result[result.length - 1] + ((result[result.length - 1] - result[result.length - 2]) / (result[result.length - 2] - result[result.length - 3])) * (result[result.length - 1] - result[result.length - 2]));
          i++;
        }
      }
      ...
      if (n[0] === 0 && n[1] === n[2]) {
        ...
        while (i < 10) {
          result.length === 0
            ? result.push(n[n.length - 1] + n[n.length - 2])
            : result.length === 1
            ? result.push(result[0] + n[n.length - 1])
      ...
      // 等比數列
      if (n[n.length - 1] === n[n.length - 2] * n[n.length - 2] && (n[n.length - 1] - n[n.length - 2] !== n[n.length - 2] - n[n.length - 3])) {
        console.log('等比數列')
        while (i < 10) {
          result.length === 0
            ? result.push(n[n.length - 1] * n[n.length - 1])
